...
| 16 | 2026-03-12 | data-model | Workspace hierarchy model | Add `type` enum + `parent_workspace_id` self-FK to existing `workspaces` table (not a separate hierarchy table) | Keeps all existing FKs valid, records stay scoped to leaf workspace, hierarchy is resolved at query time | No |
| 17 | 2026-03-12 | data-model | Intelligent Agency model | Agency is a real workspace (`type: 'agency'`) with its own records, not a virtual view | Simpler data model, standard CRUD, all existing features (pipeline, AI, etc.) work in Agency without special-casing | Yes |
| 18 | 2026-03-12 | data-model | Joint opportunity residency | Joint deals live as records in the Agency workspace; `joint_opportunity_participants` junction table links them to participating Companies/BUs | Clear ownership, avoids multi-workspace record sharing complexity, standard record lifecycle | Yes |
| 19 | 2026-03-12 | architecture | Roll-up query strategy | Company-level views resolve child workspace IDs at auth context time, pass as array to query builder using `inArray()` | Single change point (query builder), no data duplication, no materialized views needed at current scale | Yes |
| 20 | 2026-03-12 | architecture | Backward compatibility for existing workspaces | Existing workspaces default to `type: 'company'` with `parent_workspace_id: null` — zero breaking changes | Migration safety; existing users see no difference | No |
